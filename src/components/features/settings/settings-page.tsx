...
const CARD =
  "bg-brand-paper rounded-xl p-6 border border-brand-tan shadow-[0_2px_4px_rgba(0,0,0,0.05)]";
const INP =
  "w-full px-4 py-3 rounded-lg border border-brand-tan text-[0.95rem] bg-brand-cream text-brand-dark font-sans";
const LABEL = "block text-[0.85rem] font-semibold text-brand-dark mb-[6px]";
const ROW =
  "flex justify-between items-center py-[14px] border-b border-brand-light-cream";
const SELECT =
  "px-[14px] py-2 rounded-md border border-brand-tan bg-brand-cream text-[0.9rem] text-brand-dark";
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              value={email}
              readOnly
              className={cn(INP, "opacity-70 cursor-default")}
              type="email"
            />
          </div>
          <div>
          ...
          <button
            ...
            className={cn(
              ...
              isSaving
                ? "bg-brand-tan cursor-not-allowed"
      ...
      <div className={CARD}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            {
              label: "Días Activo",
              value: String(accountStats.daysActive),
              emoji: "📅",
            },
            {
              label: "Hábitos Creados",
              value: String(accountStats.habitsCreated),
              emoji: "🔁",
            },
            {
              label: "Total Completados",
              value: accountStats.totalCompleted.toLocaleString("es-MX"),
              emoji: "✅",
            },
            {
              label: "Mejor Racha",
              value: `${accountStats.bestStreak} día${accountStats.bestStreak === 1 ? "" : "s"}`,
              emoji: "🔥",
            },
          ].map((s, i) => (
            <div
              key={i}
              className="text-center p-4 bg-brand-light-cream rounded-[10px]"
            >
              <div className="text-[1.5rem] mb-1">{s.emoji}</div>
              <div className="text-[1.3rem] font-bold text-accent">
                {s.value}
              </div>
        ...
        {accountStats.habitsCreated === 0 && (
          <p className="text-xs text-brand-warm m-0 mt-3 text-center">
            Aún no has creado hábitos. Las estadísticas aparecerán conforme uses
            la app.
          </p>
        )}
      </div>
      ...
      <div className="bg-gradient-hero rounded-xl p-6 text-hero border-none">
        <div className="flex items-center gap-6">
          <div className="w-[100px] h-[100px] rounded-full bg-gradient-accent flex items-center justify-center flex-col shrink-0">
            ...
            <span className="text-[1.8rem] font-bold text-brand-dark font-serif">
              {currentLevel}
            </span>
          </div>
          <div className="flex-1">
            ...
            <p className="m-0 mb-3 text-[0.85rem] text-hero-subtle">
              {currentXP} / {xpForNextLevel} XP · {xpForNextLevel - currentXP}{" "}
      ...
      <div className={CARD}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-[10px]">
          {[
            ...
            {
              action: "Completar todos los del día",
              xp: XP_REWARDS.completeAllDaily,
            },
            ...
            {
              action: "Puntuación de Vida sube 10+",
              xp: XP_REWARDS.lifeScoreUp10,
            },
          ].map((r, i) => (
            <div
              key={i}
              className="flex items-center justify-between px-4 py-3 bg-brand-light-cream rounded-lg"
            >
              <span className="text-[0.85rem] text-brand-dark">{r.action}</span>
              <span className="text-[0.9rem] font-bold text-accent">
                +{r.xp} XP
              </span>
            </div>
          ))}
          ...
          {LEVELS.map((lvl, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-[0.75rem] font-bold text-brand-dark">
                  Nivel {i + 1}
                </div>
                <div className="text-[0.65rem] text-brand-warm">{lvl.name}</div>
                <div className="text-[0.6rem] text-brand-medium mt-[2px]">
                  {lvl.xpRequired} XP
                </div>
              </div>
            );
            ...
            <div
              ...
              className={cn(
                ...
                b.earned
                  ...
                  : "bg-brand-light-cream border-brand-tan opacity-40"
              )}
            >
              <div className="text-[2rem] mb-[6px]">{b.emoji}</div>
              <div className="text-[0.75rem] font-semibold text-brand-dark">
                {b.name}
              </div>
  ...
  const handleToggleNotifications = async () => {
    if (!notifications) {
      ...
      if (perm === "granted") {
        ...
        setNotifStatus(
          "Permiso denegado. Habilítalas en la configuración del navegador."
        );
      }
    } else {
      ...
      <div className={CARD}>
        ...
        <div className={cn(ROW, "border-b-0")}>
          <div>
            ...
            <div className="text-[0.75rem] text-brand-warm">
              Recibirás una notificación del sistema a esta hora
            </div>
          </div>
          <input
            type="time"
        ...
        {notifStatus && (
          <div
            className={cn(
              "text-[0.8rem] mt-2 font-medium",
              notifications ? "text-success" : "text-danger"
            )}
          >
            {notifStatus}
          </div>
      ...
      <div className={CARD}>
        ...
        <div className={ROW}>
          <div>
            ...
            <div className="text-[0.75rem] text-brand-warm">
              Afecta el calendario mensual del Planificador
            </div>
          </div>
          <select
            value={weekStartsOn}
